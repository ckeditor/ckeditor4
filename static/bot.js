(function (bender) {

    bender.editorBot = function (tc, editor) {
        this.testCase = tc;
        this.editor = editor;
    };

    bender.editorBot.create = function (profile, callback) {
        var creator = profile.creator || 'replace', // was $.QueryString.creator || CKTESTER.config.creator
            name = profile.name || 'test_editor';

        var tc = bender.testCase;

        var element = CKEDITOR.document.getById(name) ||
            CKEDITOR.document.getBody().append(
                CKEDITOR.dom.element.createFromHtml(
                    creator == 'replace' ?
                    '<textarea id="' + name + '"' + '></textarea>' :
                    '<div id="' + name + '"' + ' contenteditable="true"' + '></div>')
            );

        if (typeof profile.startupData == 'string')
            element[element.is('textarea') ? 'setValue' : 'setHtml'](profile.startupData);

        CKEDITOR.once('instanceReady', function (evt) {
            var ed = evt.editor;

            // Output compact output by default.
            if (!profile.formattedOutput &&
                CKEDITOR.htmlWriter &&
                ed.dataProcessor.writer instanceof CKEDITOR.htmlWriter) {

                var writer = ed.dataProcessor.writer = new CKEDITOR.htmlParser.basicWriter();
                writer.attribute = CKEDITOR.tools.override(writer.attribute, function (org) {
                    return function (name, val) {
                        ed.config.forceSimpleAmpersand && (val = val.replace(/&amp;/g, '&'));
                        return org.call(this, name, val);
                    };
                });
                writer.sortAttributes = 1;
            }

            // Accept bookmarks created e.g. by tools.setHtmlWithSelection.
            ed.filter.allow({
                span: {
                    match: function (element) {
                        var id = element.attributes.id;

                        return id && id.match(/^cke_bm_/);
                    },
                    attributes: 'id',
                    styles: 'display'
                }
            }, 'editorbot', true);
            // Add special rules.
            if (profile.allowedForTests)
                ed.filter.allow(profile.allowedForTests, 'allowedfortests');

            var bot = new bender.editorBot(tc, ed);

            // Allow all instantiation tasks to complete.
            setTimeout(function () {
                bender.runner._running ? tc.resume(function () {
                    callback(bot);
                }) : callback(bot);
            });
        }, this);

        if (profile === true)
            profile = {};

        if (!profile.config)
            profile.config = {};

        // For convenience, load here creator dedicated plugin, to avoid having them defined by test.
        var creatorPlugin = creator == 'replace' || creator == 'append' ? 'wysiwygarea' : 'floatingspace';
        var cfg = profile.config;
        cfg.extraPlugins = creatorPlugin + ',' + (cfg.extraPlugins || '');
        profile.formattedOutput && (cfg.extraPlugins += 'htmlwriter');

        CKEDITOR[creator](element, profile.config);
        bender.runner._running && tc.wait();
    };

    bender.editorBot.prototype = {
        dialog: function (dialogName, callback) {
            var tc = this.testCase,
                editor = this.editor;

            editor.on('dialogShow', function (ev) {
                ev.removeListener();

                var dialog = ev.data;

                setTimeout(function () {
                    tc.resume(function () {
                        callback.call(tc, dialog);
                    });
                }, 0);
            });

            editor.execCommand(dialogName);
            tc.wait(1000);
        },

        getData: function (fixHtml, compatHtml) {
            var data = this.editor.getData();
            fixHtml && (data = bender.tools.fixHtml(data));
            compatHtml && (data = bender.tools.compatHtml(data));
            return data;
        },

        setData: function (data, callback) {
            var tc = this.testCase,
                editor = this.editor;

            // Ensure that setData() is async.
            wait(function () {
                editor.setData(data, function () {
                    // Make sure the resume is invoked after wait.
                    resume(function () {
                        callback.call(tc);
                    });
                });
            });
        },

        /*
         * Open a rich combo dropdown, returns the combo instance.
         * @param {String} name Name of the combo button.
         * @param {Function} callback The function invoked when dropdown is shown.
         */
        combo: function (name, callback) {
            var ed = this.editor,
                tc = this.testCase;

            var combo = ed.ui.get(name);
            ed.once('panelShow', function () {
                // Make sure resume comes after wait.
                setTimeout(function () {
                    tc.resume(
                        function () {
                            callback.call(tc, combo);
                        }
                    );
                }, 0);
            });

            var item = CKEDITOR.document.getById('cke_' + combo.id);
            item = item.getElementsByTag('a').getItem(0);
            item.$[CKEDITOR.env.ie ? 'onmouseup' : 'onclick']();

            // combo panel opening is synchronous;
            tc.wait();
        },

        /**
         * Open a menu button drop down.
         * @param {String} name Name of the panel button.
         * @param {Function} callback The function invoked when panel is opened.
         */
        menu: function (name, callback) {
            var ed = this.editor,
                tc = this.testCase;

            var btn = ed.ui.get(name);
            ed.once('panelShow', function () {
                // Make sure resume comes after wait.
                setTimeout(function () {
                    tc.resume(
                        function () {
                            callback.call(tc, btn._.menu);
                        }
                    );
                }, 0);
            });

            var btnEl = CKEDITOR.document.getById(btn._.id);
            btnEl.$[CKEDITOR.env.ie ? 'onmouseup' : 'onclick']();

            // combo panel opening is synchronous;
            tc.wait();
        },

        /**
         * Open the context menu on current editor.
         * @param {Function} callback The function invoked when panel is opened.
         */
        contextmenu: function (callback) {
            var ed = this.editor,
                tc = this.testCase;

            ed.once('panelShow', function () {
                // Make sure resume comes after wait.
                setTimeout(function () {
                    tc.resume(
                        function () {
                            callback.call(tc, ed.contextMenu);
                        }
                    );
                }, 0);
            });

            // Open context menu on editable element.
            ed.contextMenu.open(ed.editable());

            // combo panel opening is synchronous;
            tc.wait();
        },

        setHtmlWithSelection: function (html) {
            bender.tools.setHtmlWithSelection(this.editor, html);
        },

        htmlWithSelection: function (html) {
            return typeof html == 'string' ?
                bender.tools.setHtmlWithSelection(this.editor, html) :
                bender.tools.getHtmlWithSelection(this.editor);
        },

        execCommand: function () {
            this.editor.execCommand.apply(this.editor, arguments);
            if (this.editor.focusManager.hasFocus &&
                this.editor.mode == 'wysiwyg') // Don't fire selectionChange in source mode, because there's no valid selection then.
                this.fireSelectionChange();
        },

        /**
         * Delegate to {@link bender.tools.focus}, on the editor instance.
         * @param callback
         */
        focus: function (callback) {
            var ed = this.editor,
                tc = this.testCase;
            bender.tools.focus(ed, function () {
                callback.call(tc);
            });
        },

        fireSelectionChange: function () {
            var sel = this.editor.getSelection(),
                currentPath = this.editor.elementPath();

            this.editor.fire('selectionChange', {
                selection: sel,
                path: currentPath,
                element: currentPath.lastElement
            });
        },

        assertInputOutput: function (input, expectedHtml, expectedData) {
            var bot = this;

            this.editor.setData(input, function () {
                resume(function () {
                    // Test html after data->html conversion.
                    assert[typeof expectedHtml == 'string' ? 'areSame' : 'isMatching'](expectedHtml, bender.tools.compatHtml(bot.editor.getSnapshot(), 0, 1), 'Editor\'s html');
                    // Test if output data are equal to input or given expectedData.
                    assert.areSame(expectedData || input, bender.tools.compatHtml(bot.getData(), 0, 1), 'Editor\'s data');
                });
            });

            wait();
        }
    };

})(bender);
