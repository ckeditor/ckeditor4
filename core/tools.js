...
( function() {
	...
	CKEDITOR.tools = {
		 ...
		 * where the last conflicted plugin has the highest priority and will be used in a warning
		 * message.
		 *
		 * ```javascript
		...
		detectPluginsConflict: function( editor, plugin, conflicted ) {
			...
			for ( var i = 0; i < conflicted.length; i++ ) {
				...
				if ( editor.plugins[ pluginName ] ) {
					conflictPlugins.push( pluginName );
				}
			}

			if ( conflictPlugins.length ) {
				CKEDITOR.warn( 'editor-plugin-conflict', {
					plugin: plugin,
					replacedWith: conflictPlugins.pop()
				} );
				return true;
			}
			return false;
