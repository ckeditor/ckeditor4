...
(function() {
  CKEDITOR.dialog.add('link', function(editor) {
    var plugin = CKEDITOR.plugins.customlink, initialLinkText;
    ...
    var targetChanged = function() {
      ...
      if (!popupFeatures || !targetName) return;
      ...
      switch (value) {
        case 'frame':
          targetName.setLabel(editor.lang.customlink.targetFrameName);
          targetName.getElement().show();
          break;
        case 'popup':
          popupFeatures.show();
          targetName.setLabel(editor.lang.customlink.targetPopupName);
          targetName.getElement().show();
          break;
        default:
    ...
    var setupParams = function(page, data) {
      if (data[page]) this.setValue(data[page][this.id] || '');
    };
    ...
    var commonLang = editor.lang.common,
      linkLang = editor.lang.customlink,
      anchors;

    return {
      title: linkLang.title,
      minWidth: (CKEDITOR.skinName || editor.config.skin) == 'moono-lisa'
        ? 450
        : 350,
      minHeight: 240,
      contents: [
                  ...
                  children: [
                    ...
                    // },
                    {
                      ...
                      validate: function() {
                        ...
                        if (
                          this.getDialog().fakeObj // Edit Anchor.
                        )
                          return true;
              ...
              children: [
                {
                  ...
                  setup: function() {
                    ...
                    this.getElement()[
                      anchors && anchors.length ? 'show' : 'hide'
                    ]();
                  },
                  children: [
                    {
                      ...
                      children: [
                          ...
                          setup: function(data) {
                            ...
                            if (anchors) {
                              for (var i = 0; i < anchors.length; i++) {
                                if (anchors[i].name) this.add(anchors[i].name);
                              }
                            }

                            if (data.anchor) this.setValue(data.anchor.name);

                            var linkType = this.getDialog().getContentElement(
                              'info',
                              'linkType'
                            );
                        ...
                        },
                        {
                          ...
                          setup: function(data) {
                            ...
                            if (anchors) {
                              for (var i = 0; i < anchors.length; i++) {
                                if (anchors[i].id) this.add(anchors[i].id);
                              }
                            }

                            if (data.anchor) this.setValue(data.anchor.id);
                          },
                          commit: function(data) {
                            if (!data.anchor) data.anchor = {};
                ...
                },
                {
                  ...
                  setup: function() {
                    this.getElement()[
                      anchors && anchors.length ? 'hide' : 'show'
                    ]();
                  }
                }
              ],
              ...
              children: [
                {
                  ...
                  setup: function(data) {
                    if (data.email) this.setValue(data.email.address);
                ...
                },
                {
                  ...
                  setup: function(data) {
                    if (data.email) this.setValue(data.email.subject);
                  },
                  commit: function(data) {
                    if (!data.email) data.email = {};
                ...
                },
                {
                  ...
                  default: '',
                  setup: function(data) {
                    if (data.email) this.setValue(data.email.body);
                  },
                  commit: function(data) {
                    if (!data.email) data.email = {};
              ...
              children: [
                ...
                },
                {
                  ...
                  commit: function(data) {
                    ...
                    data.target.name = this.getValue().replace(
                      /([^\x00-\x7F]|\s)/gi,
                      ''
                    );
                  }
      ...
      onLoad: function() {
        if (!editor.config.linkShowAdvancedTab) this.hidePage('advanced'); //Hide Advanded tab.

        if (!editor.config.linkShowTargetTab) this.hidePage('target'); //Hide Target tab.
      },
