...
(function() {
  CKEDITOR.plugins.add('customlink', {
    ...
    onLoad: function() {
      ...
      function cssWithDir(dir) {
        return template
          .replace(/%1/g, dir == 'rtl' ? 'right' : 'left')
          .replace(/%2/g, 'cke_contents_' + dir);
      }
    ...
    init: function(editor) {
          ...
          if (element && !element.isReadOnly()) {
            if (element.is('a')) {
              evt.data.dialog = element.getAttribute('name') &&
                (!element.getAttribute('href') || !element.getChildCount())
                ? 'anchor'
                : 'link';
              ...
              CKEDITOR.plugins.customlink.tryRestoreFakeAnchor(editor, element)
            ) {
      ...
      if (editor.contextMenu) {
        editor.contextMenu.addListener(function(element) {
          ...
          var anchor = CKEDITOR.plugins.customlink.tryRestoreFakeAnchor(
            editor,
            element
          );
          ...
          if (
            !anchor &&
            !(anchor = CKEDITOR.plugins.customlink.getSelectedLink(editor))
          )
            return null;
    ...
    afterInit: function(editor) {
      ...
      if (pathFilters) {
        pathFilters.push(function(element, name) {
          if (name == 'a') {
            if (
              CKEDITOR.plugins.customlink.tryRestoreFakeAnchor(
                editor,
                element
              ) ||
  ...
  function protectEmailLinkAsFunction(editor, email) {
    var plugin = editor.plugins.customlink,
  ...
  function getCompiledProtectionFunction(editor) {
    ...
    if (emailProtection && emailProtection != 'encode') {
      ...
      emailProtection.replace(/^([^(]+)\(([^)]+)\)$/, function(
        match,
        funcName,
        params
      ) {
  ...
  CKEDITOR.plugins.customlink = {
    /**
		 ...
		 *
		 *		CKEDITOR.plugins.customlink.getSelectedLink( editor );
		 *
    ...
    getEditorAnchors: function(editor) {
      var editable = editor.editable(),
        ...
        scope = editable.isInline() && !editor.plugins.divarea
          ? editor.document
          : editable,
    ...
    parseLinkAttributes: function(editor, element) {
      var href =
        ...
        '',
        compiledProtectionFunction =
          editor.plugins.customlink.compiledProtectionFunction,
      ...
      if ((javascriptMatch = href.match(javascriptProtocolRegex))) {
        if (emailProtection == 'encode') {
          href = href.replace(encodedEmailLinkRegex, function(
            match,
            protectedAddress,
            rest
          ) {
          ...
          href.replace(functionCallProtectedEmailLinkRegex, function(
            match,
            funcName,
            funcArgs
          ) {
            if (funcName == compiledProtectionFunction.name) {
              ...
              for (var i = 0; i < paramsMatchLength; i++) {
                paramVal = decodeURIComponent(
                  unescapeSingleQuote(
                    paramsMatch[i].replace(paramQuoteRegex, '')
                  )
                );
      ...
      if (element) {
        ...
        if (!target) {
          var onclick =
            element.data('cke-pa-onclick') || element.getAttribute('onclick'),
          ...
          if (onclickMatch) {
            ...
            while ((featureMatch = popupFeaturesRegex.exec(onclickMatch[2]))) {
              ...
              if (
                (featureMatch[2] == 'yes' || featureMatch[2] == '1') &&
                !(featureMatch[1] in { height: 1, width: 1, top: 1, left: 1 })
              )
        ...
        for (var a in advAttrNames) {
          ...
          if (val) advanced[advAttrNames[a]] = val;
        }

        var advName = element.data('cke-saved-name') || advanced.advName;

        if (advName) advanced.advName = advName;

        if (!CKEDITOR.tools.isEmpty(advanced)) retval.advanced = advanced;
      }
    ...
    getLinkAttributes: function(editor, data) {
      ...
      switch (data.type) {
        ...
        case 'anchor':
          ...
          break;
        case 'email':
          var email = data.email, address = email.address, linkHref;
          ...
          switch (emailProtection) {
            ...
            default:
              ...
              linkHref = [
                'javascript:',
                protectEmailLinkAsFunction(editor, email)
              ]; // jshint ignore:line
          }
      ...
      if (data.target) {
        if (data.target.type == 'popup') {
          ...
          for (var i = 0; i < featureLength; i++)
            featureList[i] =
              featureList[i] + (data.target[featureList[i]] ? '=yes' : '=no');
      ...
      if (data.advanced) {
        ...
        if (set.name) set['data-cke-saved-name'] = set.name;
      }
      ...
      if (data.advanced) CKEDITOR.tools.extend(removed, advAttrNames);
    ...
    exec: function(editor) {
      ...
      if (CKEDITOR.env.ie) {
        var range = editor.getSelection().getRanges()[0],
          link =
            (range.getPreviousEditableNode() &&
              range.getPreviousEditableNode().getAscendant('a', true)) ||
            (range.getNextEditableNode() &&
              range.getNextEditableNode().getAscendant('a', true)),
          bookmark;
    ...
    exec: function(editor) {
      ...
      if (
        ...
        (!anchor.getChildCount()
          ? CKEDITOR.plugins.customlink.tryRestoreFakeAnchor(editor, anchor)
          : anchor.is('a'))
      )
        anchor.remove(1);
      else {
        if ((anchor = CKEDITOR.plugins.customlink.getSelectedLink(editor))) {
