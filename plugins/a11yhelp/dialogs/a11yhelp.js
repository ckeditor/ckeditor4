	...
	function replaceVariables(match, name) {
		...
		if (keystrokeCodes.length) {
			return CKEDITOR.tools.array
				.map(keystrokeCodes, representKeyStroke)
				.join(" / ");
		}
		...
		for (var i = 0; i < sectionLength; i++) {
			...
			for (var j = 0; j < itemsLength; j++) {
				var item = items[j],
					...
					itemLegend =
						CKEDITOR.env.edge && item.legendEdge
							? item.legendEdge
							: item.legend;

				itemLegend = itemLegend.replace(
					variablesPattern,
					replaceVariables
				);
				...
				sectionHtml.push(
					itemTpl.replace("%1", item.name).replace("%2", itemLegend)
				);
			}
