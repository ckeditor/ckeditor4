...
for name in headingHelp blockFormatHelp inlineStyleHelp imageHelp linkHelp aboutA11yFirst gettingStarted
do
...
sed -e '/INLINESTYLEHELP\\/ {' -e 'r inlineStyleHelp.tmp' -e 'd' -e '}' setLang-1.js > setLang-2.js
sed -e '/IMAGEHELP\\/ {'       -e 'r imageHelp.tmp'       -e 'd' -e '}' setLang-2.js > setLang-3.js
sed -e '/LINKHELP\\/ {'        -e 'r linkHelp.tmp'        -e 'd' -e '}' setLang-3.js > setLang-4.js
sed -e '/ABOUTA11YFIRST\\/ {'  -e 'r aboutA11yFirst.tmp'  -e 'd' -e '}' setLang-4.js > setLang-5.js
