...
/**
 * @fileOverview The "colorbuttontest" plugin that makes it possible to assign
 *               text and background colors to editor contents.
 *
 */
CKEDITOR.plugins.add( 'colorbuttontest', {
	requires: 'floatpanel,livebutton',
	...
	init: function( editor ) {
		var config = editor.config,
			lang = editor.lang.colorbuttontest;
		...
		if ( !CKEDITOR.env.hc ) {
			...
			if ( normalizeBackground === undefined || normalizeBackground ) {
				...
				// correctly picked by colorbuttontest plugin.
		...
		function addButton( name, type, title, order, options ) {
			...
			editor.ui.add( name, CKEDITOR.UI_LIVEBUTTON, {
				label: title,
				title: title,
				modes: { wysiwyg: 1 },
				editorFocus: 0,
		...
		function renderColors( panel, type, colorBoxId ) {
			...
			for ( var i = 0; i < colors.length; i++ ) {
				...
				var colorLabel = editor.lang.colorbuttontest.colors[ colorCode ] || colorCode;
				output.push( '<td>' +
		 ...
		 *
		 * @private
		 * @member CKEDITOR.plugins.colorbuttontest
 ...
 *
 * Read more in the [documentation](#!/guide/dev_colorbuttontest)
 * and see the [SDK sample](https://sdk.ckeditor.com/samples/colorbuttontest.html).
 *
 ...
 * allowing the [Color Button](https://ckeditor.com/cke4/addon/colorbuttontest) plugin to edit these styles.
 *
