/* bender-ckeditor-plugins: pastefromword,image */
	...
	bender.test( {
		'test replace blobUrl in PFW content to base64': function() {
			...
			editor.once( 'pasteFromWord', function( evt ) {
				...
				evt.editor.once( 'saveSnapshot', function( evt ) {
					resume( function() {
						assert.beautified.html( template.replace( '{%URL%}', imgBase64 ).replace( '{%CLASS%}', '' ), evt.editor.getData() );
					} );
				}, null, null, 999 );
			} );
			...
			editor.fire( 'paste', {
				...
				dataValue: template.replace( '{%URL%}', url ).replace( '{%CLASS%}', ' class="MsoNormal"' ),
				method: 'paste'
			} );
