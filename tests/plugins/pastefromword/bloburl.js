...
( function() {
	...
	function convertBase64ToBlobUrl( image ) {
		var regExp = /^data:(.*);base64,(.*)$/;
		var base64 = image.match( regExp );
		var imageBytes = b64ToBytes( atob( base64[ 2 ] ) );

		var blob = new Blob( new Uint8Array( imageBytes ), { type: base64[ 1 ] } );

		return URL.createObjectURL( blob, base );


		function b64ToBytes( b64 ) {
			var ret = new Array( b64.length );
			for ( var i = 0; i < b64.length; i++ ) {
				ret[ i ] = b64.charCodeAt( i );
			}
			return ret;
		}
	}
	...
	bender.test( {
		'test replace blobUrl to base64': function() {
			...
			editor.fire( 'paste', {
				...
				dataValue: '<p class="MsoNormal">Hello</p>',
				method: 'paste'
			} );
			wait();
		}
