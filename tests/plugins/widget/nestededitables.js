...
( function() {
	...
	bender.test( {
		...
		'test nestedEditable auto paragraph': function() {
			var editor = this.editor;
			...
			this.editorBot.setData( '<div data-widget="autoparagraphtest" id="w1"><div id="foo"></div></div>', function() {
				...
				range.setStartAt( editable.getFirst(), CKEDITOR.POSITION_BEFORE_END );
				range.setEndAt( editable.getFirst(), CKEDITOR.POSITION_BEFORE_END );
				range.select();
