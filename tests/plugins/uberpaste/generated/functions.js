...
( function() {
	...
	bender.editor = {
		config: {
			...
			pasteFilter: null,
			uberpasteRemoveFontStyles: false,
			uberpasteRemoveStyles: false,
			allowedContent: true
		}
	};
	...
	bender.test( {
		...
		'test stack attributes': function() {
			...
			CKEDITOR.plugins.uberpaste.createAttributeStack( element, filterMock );
		...
		'test supportFields comment': function() {
			...
			assert.areSame( CKEDITOR.cleanPaste( html, this.editor, 'word' ), writer.getHtml() );
		},
		'test dissolving lists': function() {
			...
			assert.beautified.html( '<p><span style="color:red">The list below does not copy + paste correctly:</span></p>' +
				...
				CKEDITOR.cleanPaste( html, editorStub, 'word' ), { sortAttributes: true } );
		},
