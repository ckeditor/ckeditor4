...
( function() {
	...
	bender.test( {
		...
		'test apply font size (collapsed selection)': function() {
			...
			bot.combo( 'FontSize', function( combo ) {
				...
				this.wait( function() {
					...
					bot.combo( 'FontSize', function( combo ) {
						combo.onClick( fontTools.removeStyleValue );
						this.wait( function() {
		...
		'test remove font size from text': function() {
			...
			this.assertCombo( 'FontSize', fontTools.removeStyleValue, false, bot, '<p>foo@</p>' );
		},

		'test remove font family from text': function() {
			var bot = this.editorBot;
			bender.tools.selection.setWithHtml( bot.editor, '<p><span style="' + ffArial + '">[foo]</span></p>' );
			this.assertCombo( 'Font', fontTools.removeStyleValue, false, bot, '<p>foo@</p>' );
		},
		...
		'test remove font size partialy from text': function() {
			...
			this.assertCombo( 'FontSize', fontTools.removeStyleValue, false, bot, '<p>foo <span style="font-size:24px">bar</span>@</p>' );
		},
		...
		'test remove font family partially from text': function() {
			...
			this.assertCombo( 'Font', fontTools.removeStyleValue, false, bot, '<p>foo <span style="' + ffArial + '">bar</span>@</p>' );
		},
